# Manhattan and QQ Plots for Lentil Protein Quality GWAS
library("CMplot")
library("R.utils")
...
# Significant MLMM traits only
setwd("/scratch1/njohns9/AADig_GWASResultsForPlots/") # Contains GAPIT GWAS result files for significant MLMM traits and TotalAA.
...
for(i in 1:length(filenames)){
  ...
  CMplot(data[,c(1:3,6)], col=GWASCol,
         plot.type="m",threshold=c(0.01/22280,0.05/22280),threshold.lty=c(1,2),
         threshold.lwd=c(1,1), threshold.col=c("black","grey"), amplify=FALSE,
         chr.den.col=c("darkgreen", "yellow", "red"), signal.col=NULL, bin.range=c(1,50), 
         bin.size =1e6, signal.cex=1, file="jpg",memo="",dpi=600,file.output=TRUE,
         verbose=TRUE, points.alpha=255)
  renameFile("Rectangular-Manhattan.MLMM.jpg", paste0(substr(filenames[i],1,nchar(filenames[i])-4),".Manhattan.jpg"))
}

# Supplemental: Significant aminoacid-model combinations
setwd("/scratch1/njohns9/AADig_GWASResultsForPlotsSup/") #Contains GAPIT GWAS result files for significant traits across all models.
filenames <- list.files(pattern=".csv")
# filenames <- "Alanine.csv"
...
# Supplemental: For Digestibilty, because SUPER would not run with Digestibility data.
setwd("/scratch1/njohns9/AADig_GWASResultsForPlotsSup/")
filenames <- list.files(pattern=".csv")
